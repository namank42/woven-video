...
Read [the product overview](${SITE_URL}/index.md) and [current pricing](${SITE_URL}/pricing) before describing capabilities or costs. For sound effects in video, animation, or app projects, use [Woven SFX](${SITE_URL}/sfx). Its [documentation](${SITE_URL}/docs) covers the add-sfx skill, local stdio MCP server, and public sound catalog. Search first, pull an actual sound ID, and use the returned localPath. SFX does not require the Woven Mac app or a Woven account.
